...
@pytest.fixture
def sample_mft_entry_single_file() -> Path:
    p = Path(__file__).parent.parent / "samples" / "entry_single_file"
    assert p.exists()
...
@pytest.fixture
def sample_mft_multiple_index_root_entries() -> Path:
    p = Path(__file__).parent.parent / "samples" / "entry_multiple_index_root_entries"
    assert p.exists()
...
def test_iter_attributes(sample_mft_entry_single_file: Path):
